...
```powershell
.\yangyunzhao-fork\仓库维护\Sync-Upstream.ps1 -RetryCount 5 -RetryDelaySeconds 10
```

暂时不下载子模块或不推送远端：

```powershell
.\yangyunzhao-fork\仓库维护\Sync-Upstream.ps1 -SkipSubmodules
.\yangyunzhao-fork\仓库维护\Sync-Upstream.ps1 -NoPush
```
...
个人 Fork 已把官方根目录 `README.md` 重命名保留为
[`上游官方项目说明.md`](../../上游官方项目说明.md)，并在原路径创建个人
[`README.md`](../../README.md)。官方上游以后修改 `README.md` 时，这两个不同用途
的文件不会自动可靠同步，且可能在 merge 或 rebase 时产生冲突。

每次同步必须：

1. 保留个人 `README.md` 的个人 Fork 定位和两仓关系说明；
2. 单独检查 `upstream/main:README.md` 的变化；
3. 人工把官方说明更新到 `上游官方项目说明.md`；
4. 复查图片、锚点和相对链接；
5. 不使用静默 `merge=ours` 忽略官方 README 更新。
